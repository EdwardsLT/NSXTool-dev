...
//
//! @file      tests/chemistry/TestMaterial.cpp
//! @brief     Implements module/class/test TestMaterial
//!
...
int main()
{
    ...
    NSX_CHECK_CLOSE(massFractions.at("H[1]") ,  4.00000000*1.007825032*0.99985/methaneMolarMass, tolerance);
    NSX_CHECK_CLOSE(massFractions.at("H[2]") ,  4.00000000*2.014101778*0.00015/methaneMolarMass, tolerance);
    ...
    NSX_CHECK_CLOSE(atomicDensity.at("O[16]"),      nsx::avogadro*0.99762*water.massDensity()/waterMolarMass, tolerance);
    NSX_CHECK_CLOSE(atomicDensity.at("O[17]"),      nsx::avogadro*0.00038*water.massDensity()/waterMolarMass, tolerance);
    NSX_CHECK_CLOSE(atomicDensity.at("O[18]"),        nsx::avogadro*0.002*water.massDensity()/waterMolarMass, tolerance);
    NSX_CHECK_CLOSE(atomicDensity.at("H[1]") , nsx::avogadro*2.0*0.999850*water.massDensity()/waterMolarMass, tolerance);
    NSX_CHECK_CLOSE(atomicDensity.at("H[2]") , nsx::avogadro*2.0*0.000150*water.massDensity()/waterMolarMass, tolerance);
    ...
    NSX_CHECK_CLOSE(water.muIncoherent(), nsx::barn*(dO16*0.0+dO17*0.004+dO18*0.0+dH1*80.270+dH2*2.05), tolerance);
    NSX_CHECK_CLOSE(water.muAbsorption(4.8*nsx::ang), nsx::barn*(dO16*0.0001+
