...
//
//! @file      nsxlib/stl/Variant.h
//! @brief     Implements module/class/test Variant
//!
...
//! Template meta-class for computing the maximum size of the types of a Variant
template<typename T, typename... Args>
struct MaxSizeOf<T, Args...> {
    static_assert(!std::is_reference<T>::value, "Variant does not support reference types");
    static constexpr size_t value = sizeof(T) > MaxSizeOf<Args...>::value ? sizeof(T) : MaxSizeOf<Args...>::value;
};
...
//! Class that implements a variadic template variant class similar as std::variant class defined in C++17
template<typename... Args>
class Variant {

public:
    template<typename... OtherArgs>
    struct is_subset { static constexpr bool value = true; };

    template<typename T, typename... OtherArgs>
    struct is_subset<T, OtherArgs...> {
    ...
    //! Construct by value
    template<typename T>
    Variant(T&& value): _typeid(typeid(T)) {
    ...
    template<typename... OtherArgs>
    Variant(Variant<OtherArgs...>& other): _typeid(other._typeid) {
        static_assert(is_subset<OtherArgs...>::value,"");
        // Copy the other storage to the current storage
        CopyStorage<OtherArgs...>(_storage, _typeid, other._storage);
    }
    ...
    template <typename... OtherArgs>
    Variant& operator=(const Variant<OtherArgs...>& other)
    {
        static_assert(is_subset<OtherArgs...>::value,"");
    ...
    //! Copy assignment operator from a subset variant
    template <typename... OtherArgs>
    Variant& operator=(Variant<OtherArgs...>& other)
    {
        static_assert(is_subset<OtherArgs...>::value,"");
