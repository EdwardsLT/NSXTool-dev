...
//
//! @file      nsxlib/geometry/ConvexHull.cpp
//! @brief     Implements module/class/test ConvexHull
//!
...
ConvexHull& ConvexHull::operator=(const ConvexHull &other)
{
    if (this != &other) {
        ...
        for (auto p : _vertices) {
            ...
            new_vertex->_duplicate = other_vertex->_duplicate ? _edges[other_vertex->_duplicate->_id] : nullptr;        
        }
...
AABB ConvexHull::aabb() const
{
    ...
    Eigen::Vector3d lower(std::numeric_limits<double>::infinity(),
